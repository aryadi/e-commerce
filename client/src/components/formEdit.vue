<template>
  <v-container class="my-5">
    ...
    <form @submit.prevent="editProduct" enctype="multipart/form-data">
      ...
      <img :src="productObj.image" class="image-class" alt="">
      <input type="file" name="image" accept="image/png, image/jpeg" @change="parseImage($event)">
      <v-btn @click="updateImage" color="info">Submit</v-btn>
    </form>
  </v-container>
</template>
...
<script>
  ...
  export default {
    data() {
      return {
        productObj: {
          _id: this.$store.state.selectedProduct._id,
          name: this.$store.state.selectedProduct.name,
          price: this.$store.state.selectedProduct.price,
          stock: this.$store.state.selectedProduct.stock,
          description: this.$store.state.selectedProduct.description,
          image: this.$store.state.selectedProduct.image,
          parsedImage: ''
        }
      }
    },
    methods: {
      updateImage() {
        ...
        formData.append('description', this.productObj.description);
          ...
          console.log(this.productObj.parsedImage);
        axios({
            method: 'PATCH',
            url: this.$store.state.baseUrl + '/products/' + this.productObj._id,
            data: formData,
            headers: {
